...
- `fetchStatus` cho thông tin `queryFn` có đang chạy không

## Cơ chế catching

Một data mà đã `stale` thì khi gọi lại query data đó, nó sẽ fetch lại api. Nếu không `stale` thì không fetch lại api.

=> Còn đối với trường hợp staleTime giữa 2 lần khác nhau thì nếu data của lần query thứ 1 xuất hiện lâu hơn thời gian staleTime của lần query thứ 2 thì nó sẽ bị gọi lại ở lần thứ 2, dù cho có stale hay chưa. Ví dụ: useQuery({ queryKey: ['todos'], queryFn: fetchTodos, staleTime: 10*1000 }) xuất hiện 5s trước, bây giờ chúng ta gọi lại useQuery({ queryKey: ['todos'], queryFn: fetchTodos, staleTime: 2*1000 }) thì rõ ràng cái data của lần 1 dù nó chưa được cho là stale nhưng nó xuất hiện 5s trước và lâu hơn thời gian staleTime là 2s nên nó sẽ bị gọi lại ở lần 2.
